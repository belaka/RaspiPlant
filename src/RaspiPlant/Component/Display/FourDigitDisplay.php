...
namespace RaspiPlant\Component\Display;
...
class FourDigitDisplay extends AbstractDisplay {
    ...
    public function __destruct() {
        $this->device->digitalWrite(self::FOURDIGIT_ALL_OFF_CMD, $this->pin, 0, 0);
    }
    ...
    public function fourDigitInit () {
        $this->device->digitalWrite(self::FOURDIGIT_INIT_CMD, $this->pin, 0, 0);
    ...
    public function fourDigitNumber ($value, $leadingZero) {
        ...
        if ($leadingZero) {
            $this->device->digitalWrite(self::FOURDIGIT_VALUE_CMD, $this->pin, $byte1, $byte2);
        } else {
            $this->device->digitalWrite(self::FOURDIGIT_ZEROS_VALUE_CMD, $this->pin, $byte1, $byte2);
        }
    ...
    public function fourDigitBrightness ($brightness) {
        ...
        $this->device->digitalWrite(self::FOURDIGIT_BRIGHTNESS_CMD, $this->pin, $brightness, 0);
        sleep(0.05);
    ...
    public function fourDigitDigit ($segment, $value) {
        $this->device->digitalWrite(self::FOURDIGIT_INDIVIDUAL_DIGIT_CMD, $this->pin, $segment, $value);
        sleep(0.05);
    ...
    public function fourDigitSegment ($segment, $leds) {
        $this->device->digitalWrite(self::FOURDIGIT_INDIVIDUAL_LED_CMD, $this->pin, $segment, $leds);
        sleep(0.05);
    ...
    public function fourDigitScore ($left, $right) {
        $this->device->digitalWrite(self::FOURDIGIT_SCORE_CMD, $this->pin, $left, $right);
        sleep(0.05);
    ...
    public function fourDigitMonitor ($analog, $duration) {
        $this->device->digitalWrite(self::FOURDIGIT_ANALOG_READ_CMD, $this->pin, $analog, $duration);
        sleep($duration + 0.05);
    ...
    public function fourDigitOn () {
        $this->device->digitalWrite(self::FOURDIGIT_ALL_ON_CMD, $this->pin, 0, 0);
        sleep(0.05);
