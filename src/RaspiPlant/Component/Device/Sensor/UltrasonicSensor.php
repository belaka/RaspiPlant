<?php

namespace RaspiPlant\Component\Device\Sensor;

use RaspiPlant\Bundle\DeviceBundle\Protocol\I2CProtocol;
use RaspiPlant\Bundle\DeviceBundle\Protocol\ProtocolInterface;

/**
 *
 */
class UltrasonicSensor extends AbstractSensor
{

    /**
     *
     * @var boolean
     */
    protected $debug;

    /**
     *
     * @var int
     */
    protected $type;

    /**
     *
     * @var ProtocolInterface
     */
    protected $protocol;

    /**
     *
     * @var type
     */
    protected $fd;

    /**
     *
     * @var int
     */
    protected $pin;

    /**
     *
     * @param I2CProtocol $protocol
     * @param int $pin
     * @param string $name
     * @param boolean $debug
     */
    function __construct(I2CProtocol $protocol, $pin, $name, $debug = false)
    {

        $this->debug = $debug;
        $this->pin = $pin;
        $this->name = $name;
        $this->protocol = $protocol;

    }

    /**
     * @return array
     */
    public static function getFields()
    {

        return array(
            'distance' => array('min' => 0, 'max' => 1000, 'unit' => 'm')
        );
    }

    public function readSensorData()
    {

        try {

            $this->protocol->digitalWrite(I2CProtocol::ULTRASONIC_READ_CMD, $this->pin, I2CProtocol::I2C_UNUSED_VALUE, self::I2C_UNUSED_VALUE);
            usleep(100000);
            $number = $this->protocol->readBuffer(I2CProtocol::ULTRASONIC_READ_CMD, 0, 0, 32);
            $result = array_map(function ($val) {
                return hexdec($val);
            }, explode(':', $number));

            return json_encode(
                array(
                    'error' => null,
                    'distance' => ($result[2] * 256 + $result[3])
                )
            );

        } catch (\Exception $exc) {

            return json_encode(
                array(
                    'error' => $exc->getMessage(),
                    'distance' => 0
                )
            );
        }
    }
}
