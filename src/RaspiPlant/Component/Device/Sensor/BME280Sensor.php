...
class BME280Sensor extends AbstractSensor
{
    ...
    public function __construct(I2CProtocol $protocol, $pin, $name, $debug = false)
    {
        ...
        $this->pin = $pin;
        $this->name = $name;
        if ($this->protocol->readU8(self::BME280_REG_CHIPID) != 0x60) {
    ...
    protected function init()
    {
        ...
        $this->dig_t1 = $this->protocol->readU16LE(self::BME280_REG_DIG_T1);
        $this->dig_t2 = $this->protocol->readS16LE(self::BME280_REG_DIG_T2);
        $this->dig_t3 = $this->protocol->readS16LE(self::BME280_REG_DIG_T3);

        $this->dig_p1 = $this->protocol->readU16LE(self::BME280_REG_DIG_P1);
        $this->dig_p2 = $this->protocol->readS16LE(self::BME280_REG_DIG_P2);
        $this->dig_p3 = $this->protocol->readS16LE(self::BME280_REG_DIG_P3);
        $this->dig_p4 = $this->protocol->readS16LE(self::BME280_REG_DIG_P4);
        $this->dig_p5 = $this->protocol->readS16LE(self::BME280_REG_DIG_P5);
        $this->dig_p6 = $this->protocol->readS16LE(self::BME280_REG_DIG_P6);
        $this->dig_p7 = $this->protocol->readS16LE(self::BME280_REG_DIG_P7);
        $this->dig_p8 = $this->protocol->readS16LE(self::BME280_REG_DIG_P8);
        $this->dig_p9 = $this->protocol->readS16LE(self::BME280_REG_DIG_P9);

        $this->dig_h1 = $this->protocol->readU8(self::BME280_REG_DIG_H1);
        $this->dig_h2 = $this->protocol->readS16LE(self::BME280_REG_DIG_H2);
        $this->dig_h3 = $this->protocol->readU8(self::BME280_REG_DIG_H3);
        $this->dig_h6 = $this->protocol->readS8(self::BME280_REG_DIG_H7);

        $h4 = $this->protocol->readS8(self::BME280_REG_DIG_H4);
        $h4 = ($h4 << 24) >> 20;
        $this->dig_h4 = $h4 | ($this->protocol->readS8(self::BME280_REG_DIG_H5) & 0x0F);

        $h5 = $this->protocol->readS8(self::BME280_REG_DIG_H6);
        $h5 = ($h5 << 24) >> 20;
        $this->dig_h5 = $h5 | ($this->protocol->readS8(self::BME280_REG_DIG_H5) >> 4 & 0x0F);


        //writeRegister
        $this->protocol->write8(self::BME280_REG_CONTROLHUMID, 0x05);  //Choose 16X oversampling
        $this->protocol->write8(self::BME280_REG_CONTROL, 0xB7);  //Choose 16X oversampling
    }
    ...
    protected function readRawHumidity()
    {
         ...
         */
        $msb = $this->protocol->readU8(self::BME280_REG_HUMIDITYDATA);
        $lsb = $this->protocol->readU8(self::BME280_REG_HUMIDITYDATA + 1);
        $raw = ($msb << 8) | $lsb;
    ...
    {
        $raw = $this->protocol->readU16BE($register);
        $raw <<= 8;
        $raw = $raw | $this->protocol->readU8($register + 2);
        $raw >>= 4;
